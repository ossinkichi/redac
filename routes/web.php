...
Route::middleware('guest')->group(function () {
    Route::get('/', [UserController::class, 'loginView'])->name('login.page');

    Route::post('/auth', [UserController::class, 'authenticateLogin'])->name('login.auth');
});
...
Route::middleware(EnsureIsStudent::class)->group(function () {
    Route::get('student/feed', function ($user) {
...
Route::get('professor', function () {
    $user = Auth::user();
    $teacher = app(TeacherController::class)->show($user->user);
    $room = app(RoomSubjectTeacherController::class)->showByTeacher($teacher['id']);
...
Route::middleware(EnsureIsSecretary::class)->group(function () {
    Route::get('secretaria', function () {
        return view('secretary.secretary.dashboard', ['courses' => app(CourseController::class)->index()]);
    })->name('secretary.home');

    Route::get('secretaria/{curso}/salas', function ($course) {
        return view('secretary.secretary.room.listing', ['rooms' => app(RoomController::class)->show($course), 'course' => $course]);
    ...
    Route::get('secretaria/{course}/sala/{room}', function ($room, $course) {
        return view('secretary.secretary.room.show', ['room' => $room, 'course' => $course, 'students' => app(StudentController::class)->getRoom($course, $room), 'subjects' => app(RoomSubjectTeacherController::class)->showByRoom($room)]);
    })->name('secretary.room.show');

    Route::get('secretaria/{course}/sala/{room}/adicionar-materia', function ($course, $room) {
        return view('secretary.teacher.listing-and-select', [$course, 'room' => $room, 'teachers' => app(TeacherController::class)->index(), 'subjects' => app(SubjectController::class)->index()]);
    ...
    Route::get('secretaria/{course}/sala/{room}/adicionar-aluno', function ($course, $room) {
        return view('secretary.secretary.room.students', [
            'course' => $course,
            'room' => $room,
            'students' => app(StudentController::class)->filterCourseToEnterInRoom($course, $room),
        ]);
    })->name('secretary.room.assignStudent');
    ...
    Route::get('secretaria/registrar/materia', function () {
        return view('secretary.secretary.subject.register', ['subjects' => app(SubjectController::class)->index()]);
